...
class RealSenseCamMaster():
    def __init__(self, cam):
        ...
        #self.config.enable_stream(rs.stream.depth, 640, 480, rs.format.any, 30)
        #self.config.enable_stream(rs.stream.color, 640, 480, rs.format.any, 30)

        self.profile = self.pipe.start(self.config)
        self.depth_sensor = self.profile.get_device().first_depth_sensor()

        if self.depth_sensor.supports(rs.option.inter_cam_sync_mode):
            print("bebin {}".format(self.id))
    ...
    def post_processing(self):
        ...
        bg_removed = np.where((depth_image_3d > self.clipping_distance) | (depth_image_3d <= 0), grey_color, self.color_img)
